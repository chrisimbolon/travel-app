...
class CreateRouteUseCase:
    ...
    async def execute(
        ...
        existing = await self.repo.find_by_origin_destination(origin, destination)
        if existing:
        ...
        return await self.repo.save(route)
...
class ListRoutesUseCase:
    ...
    async def execute(self, limit: int = 50, offset: int = 0) -> list[Route]:
...
class CreateTripUseCase:
    ...
    async def execute(
        ...
        return await self.trip_repo.save(trip)
...
class GetTripUseCase:
    ...
    async def execute(self, trip_id: UUID) -> Trip:
...
class ListTripsUseCase:
    ...
    async def execute(
        ...
        if route_id:
            return await self.repo.list_by_route(
                route_id, status=status, limit=limit, offset=offset
            )
        if operator_id:
            return await self.repo.list_by_operator(
                operator_id, status=status, limit=limit, offset=offset
            )
...
class UpdateTripStatusUseCase:
    ...
    async def execute(
        self, trip_id: UUID, new_status: TripStatus, operator_id: UUID
    ) -> Trip:
        trip = await self.repo.get_by_id(trip_id)
        if not trip:
            raise DomainError(f"Trip {trip_id} not found")
        ...
        return await self.repo.update(trip)
...
class AssignDriverUseCase:
    ...
    async def execute(
        self, trip_id: UUID, driver_id: UUID, operator_id: UUID
    ) -> Trip:
        trip = await self.repo.get_by_id(trip_id)
        if not trip:
            raise DomainError(f"Trip {trip_id} not found")
        ...
        if trip.status not in {TripStatus.SCHEDULED, TripStatus.BOARDING}:
            raise DomainError("Cannot assign driver after departure")
